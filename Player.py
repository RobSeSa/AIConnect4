...
class AIPlayer:
    ...
    def get_alpha_beta_move(self, board):
        ...
        for action in Actions(board):
            ...
            if value > best_value:
                best_value = value
                best_action = action
        print 'The best action is ', best_action,' for Player', self.player_number,' with best future value:', best_value
        return best_action
    ...
    def my_pieces_eval(self, board):
        ...
        opp_two = '{0}{0}'.format(other_player(self.player_number))
        opp_three = '{0}{0}{0}'.format(other_player(self.player_number))
        opp_four = '{0}{0}{0}{0}'.format(other_player(self.player_number))
        to_str = lambda a: ''.join(a.astype(str))
        two_value= 5
        three_value= 10
        four_value= 1000
        ...
        def check_horizontal(b):
            ...
            for row in b:
                ...
                if four in to_str(row):
                    points += four_value
                if opp_two in to_str(row):
                    points -= two_value*weight
                if opp_three in to_str(row):
                    points -= three_value*weight
                if opp_four in to_str(row):
                    points -= four_value*weight
            return points
        ...
        def check_diagonal(b):
            ...
            for op in [None, np.fliplr]:
                ...
                if four in to_str(root_diag):
                    points += four_value
                if opp_two in to_str(root_diag):
                    points -= two_value*weight
                if opp_three in to_str(root_diag):
                    points -= three_value*weight
                if opp_four in to_str(root_diag):
                    points -= four_value*weight
